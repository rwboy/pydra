...
class Framework(cmd.Cmd):
    ...
    def onecmd(self, line):
        cmdline, arg, line = self.parseline(line)
        if not line:
        ...
        if cmdline == '':
            ...
            try:
                func = getattr(self, 'do_' + cmdline)
            except AttributeError:
                return self.default(line)
            return func(arg)

    # make help menu more attractive
    def print_topics(self, header, mycmds, cmdlen, maxcol):
        if mycmds:
            self.stdout.write("%s\n" % str(header))
            if self.ruler:
                self.stdout.write("%s\n" % str(self.ruler * len(header)))
            for mycmd in mycmds:
                self.stdout.write("%s %s\n" % (mycmd.ljust(15), getattr(self, 'do_' + mycmd).__doc__))
            self.stdout.write("\n")
    ...
    def error(self, line):
        ...
        print_red('[!]'+self.to_unicode(line))

    def output(self, line):
        '''Formats and presents normal output.'''
        print_blue('[*]'+self.to_unicode(line))

    def alert(self, line):
        '''Formats and presents important output.'''
        print_green('[*]'+self.to_unicode(line))
